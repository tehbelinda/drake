...
void SphereDistanceInSphereFrame(const fcl::Sphered& sphere,
  ...
  *grad_S = *is_grad_W_unique ? p_SQ / dist_GQ : Gx;
...
void ComputeDistanceToPrimitive(const fcl::Capsuled& capsule,
  ...
  if (p_GQ.z() >= half_length || p_GQ.z() <= -half_length) {
    ...
    *p_GN = p_GS_G + p_SN_G;
    ...
    Vector3<T> p_GM_G;
    Vector3<T> grad_G;
    SphereDistanceInSphereFrame(sphere_S, p_GR, &p_GM_G, distance, &grad_G,
                                is_grad_W_unique);
    *p_GN << p_GM_G.x(), p_GM_G.y(), p_GQ.z();
    *grad_W = X_WG.rotation() * grad_G;
  }
}
