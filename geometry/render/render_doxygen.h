...
/** @addtogroup render_engines
 ...
 Drake includes *two* implementations of that API:
   ...
   - RenderEngineVtk - an GPU-based rasterization renderer
 ...
 The current API is simple and precludes sophisticated definitions of lights and
 materials. As the API matures, these additional features are added the fidelity
 differences between the two RenderEngine implementations will increase.
 ...
 The performance of RenderEngienVtk will depend on the quality of the GPU and
 its driver. A more power graphics card with an up-to-date driver will likely
 ...
 *scene complexity*.
 ...
 scalability as a simulation includes multiple cameras.

 The output image can be configured to arbitrary size. For both RenderEngine
 ...
 ```
   ...
   - __TestName__: One of
     ...
     - __OsprayPathColor__: Renders the color image from RenderEngineOspray with
       path-traced global illumination (but with only a single sample per
       pixel).
